            ...
            <div className='flex items-center gap-10'>
                ...
                
                {
                    ["Home", "Work", "Culture", "", "News"].map((elem, index) => (
                        <a key={index} href="#" className='font-normal text-md flex items-center gap-1 transition-scale hover:scale-105 hover:drop-shadow-sm'>
                            {elem.length == 0 && <span className='inline-block w-[1px] h-6 cursor-none bg-gray-400'></span>}
                            {index == 1 && <span className='inline-block h-2 w-2 rounded-full bg-orange-400'></span>}
                            {elem}
                        </a>
                    ))
                }
